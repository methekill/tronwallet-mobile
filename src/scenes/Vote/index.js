...
class VoteScene extends Component {
  ...
  _refreshCandidates = async (refreshControl = true) => {
    ...
    try {
      ...
      store.write(() =>
        voteList.map(item => store.create('Candidate', item, true))
      )
      this.setState({
        totalVotes,
      ...
      if (userVotes) {
        ...
        const newTotalRemaining =
          totalFrozen - currentUserVoteCount >= 0
            ? totalFrozen - currentUserVoteCount
            : 0
  ...
  _filteredSuggestions = () => {
    const candidateTronWallet = this.candidateStoreRef
      .objects('Candidate')
      .filtered("name = 'TronWalletMe'")[0]
    const mostVoted = this.state.currentFullVotes.filter(
      candidate => candidate.name !== 'TronWalletMe'
    )
